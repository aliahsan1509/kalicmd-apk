...
class Section(Screen):
    ...
    def select_item(self, name):
        ...
        elif self.section == "progress":
            self.app.open_progress()
        else:
            show_msg(
                name,
...
class ProgressScreen(Screen):
    def __init__(self, app, **kwargs):
        ...
        top = BoxLayout(size_hint=(1, None), height=50, spacing=6)
        back = Button(text="‹ Back", size_hint=(0.22, 1))
        back.bind(on_press=lambda *_: setattr(app.sm, "current", "home"))
        top.add_widget(back)
        top.add_widget(title("📈 Your Progress", 19))
        root.add_widget(top)
    ...
    def refresh(self):
        ...
        total = len(LINUX_FUNDAMENTALS["lessons"])
        done = sum(
            1 for lesson in LINUX_FUNDAMENTALS["lessons"]
            if lesson["id"] in self.app.completed_lessons
        )
        percent = int((done / total) * 100) if total else 0

        self.body.add_widget(Label(
            text=f"[b]Linux Fundamentals[/b]\\n\\n"
                 f"Completed: {done}/{total} lessons\\n"
                 f"Progress: {percent}%\\n"
                 f"XP: {self.app.xp}",
            markup=True, size_hint_y=None, height=125,
            text_size=(420, None), halign="left", valign="top"
        ...
        bar_width = max(1, int(34 * percent / 100))
        bar = "█" * bar_width + "░" * (34 - bar_width)
        self.body.add_widget(Label(
            text=bar, size_hint_y=None, height=42,
            font_size="18sp", halign="center"
        ))

        self.body.add_widget(Label(
            text="[b]Lesson status[/b]", markup=True,
            size_hint_y=None, height=38, halign="left"
        ))

        for index, lesson in enumerate(LINUX_FUNDAMENTALS["lessons"], 1):
            is_done = lesson["id"] in self.app.completed_lessons
            ...
            b.bind(
                on_press=lambda _b, data=lesson:
                    self.app.open_linux_lesson(data)
            )
...
class KaliApp(App):
    def build(self):
        ...
        self.sm.add_widget(ProgressScreen(self, name="progress-screen"))
        return self.sm
    def load_data(self):
        try:
    ...
    def open_progress(self):
        self.sm.current = "progress-screen"
        self.sm.get_screen("progress-screen").refresh()
    ...
    def open_section(self, section):
        if section == "tools":
            ...
            self.sm.get_screen("tools").rebuild(
                [t for t in TOOLS if t["name"] in self.favs]
            )
