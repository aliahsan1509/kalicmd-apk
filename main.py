...
class Section(Screen):
    ...
    def __init__(self, app, section, **kwargs):
        ...
        for item in items:
            ...
            b.bind(on_press=lambda _b, name=item: show_msg(name, "This module is planned and will be built step-by-step.\n\nFor now, explore Tools."))
            body.add_widget(b)
...
class KaliApp(App):
    def build(self):
        ...
        self.favs, self.xp = set(), 0; self.load_data()
        ...
        for s in ["learning", "concepts", "labs", "missions", "cheats", "progress", "ai"]: self.sm.add_widget(Section(self, s, name=s))
        return self.sm
    def load_data(self):
        try:
            with open(self.fav_file, encoding="utf-8") as f: d = json.load(f); self.favs = set(d.get("favorites", [])); self.xp = int(d.get("xp", 0))
        except Exception: pass
    def save_data(self):
        try:
            os.makedirs(self.user_data_dir, exist_ok=True)
            with open(self.fav_file, "w", encoding="utf-8") as f: json.dump({"favorites": sorted(self.favs), "xp": self.xp}, f)
        except Exception: pass
